...
import {
  NazivProjekta
} from '../enums/naziv-projekta.enum';
import {
  IteracijaProjekta
} from '../models/iteracija-projekta';
...
export class IteracijaProjektaService {
  ...
  create(naziv_projekta: NazivProjekta, godina: number): Observable < IteracijaProjekta > {
    return this.http.post < IteracijaProjekta > (this.API_ENDPOINT, {
      naziv_projekta,
      godina
    });
  }
  ...
  findAllByNaziv(naziv: NazivProjekta): Observable < IteracijaProjekta[] > {
    return this.http.get < IteracijaProjekta[] > (`${this.API_ENDPOINT}/sve`, {
      params: {
        naziv
      },
    });
